...
int main()
{
    ...
    uint16_t win_h2 = 500;
    uint16_t win_w2 = 500;
    float spacing_both[2] = {25.0f, 25.0f};

    config_window(&window1, win_h1, win_w1, "window1", RED, spacing_both);
    config_window(&window2, win_h2, win_w2, "window2", BLUE, spacing_both);
    ...
    set_orthographic_projection(grid_program_1, 0.0f, 0.0f, win_w1, win_h1);
    windows_array[0]->grid_data = build_grid(grid_program_1, win_w1, win_h1, spacing_both[0], spacing_both[1]);

    //win 2
    glfwMakeContextCurrent(windows_array[1]->window);
    initialize_glad();
    glViewport(0, 0,windows_array[1]->width,windows_array[1]->height);
    uint32_t grid_program_2 = create_shader_program(
        "shader/GridShader.vert",
      "shader/GridShader.frag"
        );

    set_orthographic_projection(grid_program_2, 0.0f, 0.0f, win_w2, win_h2);
    windows_array[1]->grid_data = build_grid(grid_program_2, win_w2, win_h2, spacing_both[0], spacing_both[1]);
    ...
    for (uint32_t i = 0; i < amount_of_windows; i++)
    {
